...
describe("Arbitrum Migration Contract", function () {
    ...
    beforeEach(async () => {
        ...
        const migrationFactory = await ethers.getContractFactory("ArbitrumTokenMigration");
    ...
    describe("migration", async () => {
        ...
        it("reverts if there is insufficient MYC balance", async () => {
            ...
            await expect(
                migrationContract
                    .connect(signers[3])
                    .migrate(ethers.utils.parseEther("250"))).to.be.revertedWith("ERC20: transfer amount exceeds balance")
        })
    ...
    describe("withdrawTokens", async() => {
        it("supports withdrawing excess MYC", async() => {
            ...
            expect(balanceAfter.toString()).to.equal(ethers.utils.parseEther("0").toString())
            expect(diff.toString()).to.equal(ethers.utils.parseEther("100").toString())
        })
    })
});
