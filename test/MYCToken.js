...
describe("Migration Contract", function () {
    ...
    describe("minting", async() => {
        it("allows minter role to mint if minting is NOT paused", async() => {
            ...
            await MYC.connect(signers[2]).mint(signers[5].address, ethers.utils.parseEther("100"))
            let balAfter = await MYC.balanceOf(signers[5].address)
            let supplyAfter = await MYC.totalSupply()

            expect(balAfter.sub(balBefore).toString()).to.equal(ethers.utils.parseEther("100").toString())
            expect(supplyAfter.sub(supplyBefore).toString()).to.equal(ethers.utils.parseEther("100").toString())
        })

        it("reverts if minting is paused", async() => {
            await MYC.connect(signers[2]).setMintingPaused(true)
            await expect(
                MYC.connect(signers[2]).mint(signers[5].address, ethers.utils.parseEther("100"))
            ).to.be.revertedWith("MINTING_PAUSED")
        })

        it("reverts if non minter role", async() => {
            await expect(
                MYC.connect(signers[3]).mint(signers[5].address, ethers.utils.parseEther("100"))
    ...
    describe("setMintingPaused", async() => {
        ...
        it("allows the minting pauser to pause minting", async() => {
            await MYC.connect(signers[2]).grantRole(ethers.utils.id("MINTING_PAUSER"), signers[5].address)
            let isMintingPaused = await MYC.mintingPaused()
        ...
        it("reverts if non minting pauser or default admin", async() => {
            expect(
                MYC.connect(signers[7]).setMintingPaused(true)
            ).to.be.revertedWith("NOT_PAUSER")
        })
    })
});
