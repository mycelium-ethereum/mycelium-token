...
describe("Migration Contract", function () {
    ...
    describe("migration", async() => {
        ...
        it("migrates to msg.sender if no to address provided", async() => {
            ...
            await TCR.connect(signers[3]).approve(migrationContract.address, ethers.utils.parseEther("100"))
            await migrationContract.connect(signers[3]).migrate(ethers.utils.parseEther("100"))
            let MYCBalAfter = await MYC.balanceOf(signers[3].address)
            let diffBal = MYCBalAfter.sub(MYCBalBefore)
            expect(diffBal.toString()).to.equal(ethers.utils.parseEther("100").toString())
        })

        it("migrates to the to address", async() => {
            let MYCBalBefore = await MYC.balanceOf(signers[4].address)
            await TCR.connect(signers[3]).approve(migrationContract.address, ethers.utils.parseEther("100"))
            await migrationContract.connect(signers[3]).migrateTo(ethers.utils.parseEther("100"), signers[4].address)
